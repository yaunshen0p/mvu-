...
describe('ResultSheet - Artifact Formatting', () => {
  beforeEach(() => {
...
arr.forEach(item => {
  ...
  describe('Export payload structure', () => {
    it('should build export payload with all artifacts', () => {
      const store = useWorkspaceStore()
      ...
      expect(payload.artifacts.javascript).toBe('console.log("test");')
      expect(payload.artifacts.yaml).toBe('key: value')
      expect(payload.artifacts.script).toBe('const x = 1;')
      expect(payload.artifacts.regex).toBe('/test/gi')
    })
